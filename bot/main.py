...
async def course_category_parent(callback_query: types.CallbackQuery):
    ...
    if category_parent:
        ...
        await callback_query.message.answer(text="Hozircha Bu yonalish boyicha kurs qoshilmadi")
    ...
    if data:
        for i in data:
            with open(f"../media/{i}", 'rb') as video:
                print(video)
                await callback_query.message.answer(text="Video Dowland")
                await callback_query.bot.send_video(chat_id=callback_query.message.chat.id, video=video)
    await callback_query.message.answer(text="Not Video")
